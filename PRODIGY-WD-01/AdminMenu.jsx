...
const AdminMenu = () => {
  const initialThemes = [
    { id: 1, name: 'Dish One', image: 'image1.jpg', description: 'Description for Dish One', price: '100' },
  ]
    ...
    <div>
      ...
      <div className="bg" style={backgroundImageStyle}>
        <div className="formm-container" style={{ textAlign: 'center' }}>
          {displayTable ? (
            <div>
              <h1 className='user-tab-title'>Menu Details</h1>
              <table className='user-tab'>
                <thead>
                  <tr>
                    <th className='user-tab-head'>Dish Name</th>
              ...
              <button className='buttt' onClick={handleAddTheme}>Add Dish</button>
              <br />
              <button className='user-back'><Link to="/Adminlogin">Go Back</Link></button>
            </div>
          ) : (
            <div>
              <h1 className='user-tab-title'>{selectedThemeId ? 'Update Menu' : 'Add Theme'}</h1>
